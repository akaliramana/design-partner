...
> 1. `design-system/MASTER.md` is the single source of truth — generated by the search engine.
> 2. Every token choice must include reasoning ("why" tied to project context).
> 3. CSS variables in `globals.css` must come FROM MASTER.md, not from general knowledge.
> 4. Build only what's asked for — no preemptive template generation.
...
```
assets/designs/slides/pitch-251223.html
```
