...
After generation, **ALWAYS** ask user about HTML preview via `AskUserQuestion`. If yes, invoke `/ui-ux` for gallery.
...
2. **Research** — Activate `ui-ux`, browse Pinterest for references
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `ui-ux`, `brand`, `design-system`, `chrome-devtools` skills.
...
4. **Design** — `/ui-ux:brand` → `/ui-ux:design-system` → randomly invoke `/ui-ux` OR `/ui-ux:frontend-design`; HTML per idea × size
...
**Related Skills:** frontend-design, ui-ux, ai-multimodal, chrome-devtools
