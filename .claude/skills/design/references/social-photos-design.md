...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux`, `brand`, `design-system`, and `chrome-devtools` skills.
...
1. **`/ui-ux:brand`** — Extract brand colors, fonts, voice from user's project
2. **`/ui-ux:design-system`** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** `/ui-ux` OR `/ui-ux:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
